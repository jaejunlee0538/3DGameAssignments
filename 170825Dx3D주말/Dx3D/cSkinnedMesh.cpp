...
{
	m_pAnimController->SetTrackWeight(0, m_TrackWeight0);
	m_pAnimController->SetTrackWeight(1, m_TrackWeight1);

	if (m_TrackWeight1 > 0.1f)
	{
		m_TrackWeight0 = 1 - m_TrackWeight1;
		m_TrackWeight1 -= 0.1f;
		m_pAnimController->SetTrackPosition(1, m_dPrevPosition);
	}
	else
	{
...
void cSkinnedMesh::SetAnimationIndex(int nIndex)
{
	if (!m_pAnimController)
		return;
	if (m_PrevAnimationIndex == nIndex) return;
	if (m_playAni) return;
	m_PrevAnimationIndex = nIndex;
	if (m_PrevAnimationIndex == 2)
	{
 		m_playAni = true; 
	}
	...
	//현재 애니메이션을 이전 애니메이션 셋에 넣고 1번 트랙에 세팅
	//m_pAnimController->GetAnimationSet(m_PrevAnimationIndex, &pPrevAnimationSet);
	m_pAnimController->GetTrackAnimationSet(0, &pPrevAnimationSet);

	D3DXTRACK_DESC TrackDesc_0;
	D3DXTRACK_DESC TrackDesc_1;
	m_pAnimController->GetTrackDesc(0, &TrackDesc_0);
	m_pAnimController->GetTrackDesc(1, &TrackDesc_1);

	m_pAnimController->SetTrackDesc(1, &TrackDesc_0);
	m_dPrevPosition = TrackDesc_1.Position;

	m_pAnimController->SetTrackAnimationSet(1, pPrevAnimationSet);

	m_pAnimController->GetAnimationSet(nIndex, &pNextAnimationSet);
	m_pAnimController->SetTrackAnimationSet(0, pNextAnimationSet);

	m_pAnimController->SetTrackPosition(0, 0);	//0번트랙 진행상태를 처음으로

	m_dPeriod = pNextAnimationSet->GetPeriod();//현재 애니메이션 구간 길이를 저장
...
bool cSkinnedMesh::GetHalf(void)
{
	LPD3DXANIMATIONSET pAnimSet;
	m_pAnimController->GetAnimationSet(m_AnimationIndex, &pAnimSet);
	D3DXTRACK_DESC TrackDesc_0;
	m_pAnimController->GetTrackDesc(0, &TrackDesc_0);
	m_dPosition = TrackDesc_0.Position;

	float fHalf = (m_dPosition / m_dPeriod);
