...
namespace SGA {
	...
	{
		return{ (int)std::roundf(pt.x), (int)std::round(pt.y) };
	}
	...
	void getRotatedRECT(const RECT & rect, const float & angle, POINTFLOAT * pt, const POINTFLOAT & pivot)
	{
		pt[0].x = rect.left;	pt[1].x = rect.right;	pt[2].x = rect.right;		pt[3].x = rect.left;
		pt[0].y = rect.top;		pt[1].y = rect.top;		pt[2].y = rect.bottom;		pt[3].y = rect.bottom;

		for (int i = 0; i < 4; i++) {
			rotate2D(pt[i], angle, pivot);
		}
	}
	...
	{
		POINTFLOAT pt[4];
		getRotatedRECT(rect, angle, pt, POINT2POINTFLOAT(pivot));
		for (int i = 0; i < 4; i++) {
			points[i] = POINTFLOAT2POINT(pt[i]);
		}
	}
