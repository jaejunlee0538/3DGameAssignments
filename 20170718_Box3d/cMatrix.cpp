...
cMatrix cMatrix::CameraPosition(const cVector3 & vEye, const cVector3 & vLookAt, const cVector3 & vUp)
{
	cMatrix mat = cMatrix::Identity(4);
...
cMatrix cMatrix::InverseTransformationMatrix(const cMatrix & m)
{
	cMatrix inv = cMatrix::Identity(4);
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			inv(j, i) = m(i, j);
		}
	}
	cVector3 p(m(3, 0), m(3, 1), m(3, 2));
	inv(3, 0) = -cVector3::Dot(p, cVector3(m(0, 0), m(0, 1), m(0, 2)));
	inv(3, 1) = -cVector3::Dot(p, cVector3(m(1, 0), m(1, 1), m(1, 2)));
	inv(3, 2) = -cVector3::Dot(p, cVector3(m(2, 0), m(2, 1), m(2, 2)));
	return inv;
}

cMatrix cMatrix::NormalizeRotationPart(const cMatrix & m)
{
	cMatrix normalized = m;
	for (int i = 0; i < 3; ++i) {
		cVector3 v(m(i, 0), m(i, 1), m(i, 2));
		v = v.Normalize();
		normalized(i, 0) = v[0];
		normalized(i, 1) = v[1];
		normalized(i, 2) = v[2];
...
void LogcMatrix(const cMatrix & m, const TCHAR * name)
{
	...
	oss << m;
	LogDebugMessage(TEXT("%s = \n %s"), name, SGA::convertToTString(oss.str()).c_str());
	//LogDebugMessageA("%s = \n %s", name, oss.str().c_str());
}
