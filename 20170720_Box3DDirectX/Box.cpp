...
void Box::Render() 
{
	...
	g_pD3DDevice->DrawPrimitiveUP(
		D3DPT_TRIANGLELIST, 12, _vertices, sizeof(tagVertex));
}

void Box::Update(SGA::UserInputData & input)
{
...
D3DXVECTOR3 Box::GetPosition() const
{
	return D3DXVECTOR3(_matWorld(3, 0), _matWorld(3, 1), _matWorld(3, 2));
}

void Box::Rotate(float angleX, float angleY, float angleZ)
{
