...
namespace SGA {
	namespace ModelLoad {
		...
		void OBJLoader::load3DModel(const TCHAR * modelFile, MeshModel::Ptr& meshModel)
		{
			_position.clear(); 
			_normal.clear();
			_uv.clear();
			...
			while (std::getline(file, buffer)) {
				...
				if (token == "v") {
					...
					if (token.empty() == false) {
						//그룹 이름이 있는 경우
						//새로운 그룹을 생성한다.
						group.reset(new ModelGroup());
						group->name = token;
					}
				}
				else if (token == "usemtl") {
					assert(group && "ModelGroup이 생성되지 않았습니다.");
					iss >> token;
		...
		void OBJLoader::loadMaterialLibrary(const char * mtlFile){
			...
			while (std::getline(file, buffer)) {
				...
				if (token == "newmtl") {
					...
					iss >> token;
					material->texture = GET_TEXTURE_MGR()->createTexture(convertToTString(token).c_str());
					material = nullptr;
				}
				else {
					//???
