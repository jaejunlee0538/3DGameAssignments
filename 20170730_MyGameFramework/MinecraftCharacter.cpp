...
namespace SGA {
	Minecrafcharacter::Minecrafcharacter()
	{
		_time = 0;
	}
	...
	void Minecrafcharacter::setup(D3DTexturePtr texture)
	{
		...
		m_head->setScale(scale, scale, scale);
		m_body->setScale(scale, scale, scale);
		m_armL->setScale(scale, scale, scale);
		m_armR->setScale(scale, scale, scale);
		m_legL->setScale(scale, scale, scale);
		m_legR->setScale(scale, scale, scale);
	...
	void Minecrafcharacter::update(bool isDirty)
	{
		_time += 1.0 / 60.0f;
	...
	void Minecrafcharacter::render() const{
		g_pD3DDevice->SetMaterial(&_materialCube);
		GameObject::render();
	}

	void Minecrafcharacter::setState(States state)
	{
		if (state != _state) {
			switch (state) {
			case STATE_IDLE:
	...
	void Minecrafcharacter::setAnimation(const char * animName)
	{
		_currentAnimation = &_animations[animName];
	}

	void Minecrafcharacter::initAnimation()
	{
		...
		appendKeyFramesRotation(animWalk.boneAnimations[1].keyFrames, 0, 1, 0.03, D3DXVECTOR3(0, D3DXToRadian(360), 0), D3DXVECTOR3(0, 0, 0), D3DXVECTOR3(0, 0.05, 0), D3DXVECTOR3(0, 0, 0));
	...
	void Minecrafcharacter::initAnimationClip(AnimationClip & animClip)
	{
	...
	void Minecrafcharacter::appendKeyFramesRotation(KeyFrames & keyFrames, float tBegin, float tEnd, float dt, D3DXVECTOR3 angularSpeed, D3DXVECTOR3 angleOffset, D3DXVECTOR3 scale, D3DXVECTOR3 magnitudeOffset)
	{
	...
	void Minecrafcharacter::appendKeyFramesLinearInterpolate(KeyFrames & keyFrames, float tBegin, float tEnd, float dt, D3DXVECTOR3 posBegin, D3DXVECTOR3 posEnd, D3DXVECTOR3 angleBegin, D3DXVECTOR3 angleEnd)
	{
		size_t nFrames = (tEnd - tBegin) / dt + 1;
		keyFrames.reserve(keyFrames.size() + nFrames);
		float tOffset = keyFrames.empty() ? 0 : keyFrames.back().time;
