...
void cObjLoader::LoadMesh(IN char * szFullPath, D3DXMATRIX* vertMat, OUT cFrame** meshObject){
	vector<D3DXVECTOR3> vecV;
	vector<D3DXVECTOR2> vecVT;
	vector<D3DXVECTOR3> vecVN;
	vector<ST_PNT_VERTEX> vecVertex;
	...
	while (!feof(fp))
	{
		...
		else if (szBuf[0] == 'g')
		{
			if (!vecVertex.empty())
			{
				m_meshCreator.AddFaces(vecVertex, szMtlName);
				vecVertex.clear();
			}
		}
		else if (szBuf[0] == 'v')
		...
		else if (szBuf[0] == 'f')
		{
			...
			for (size_t i = 0; i < 3; ++i)
			{
				...
				D3DXVECTOR3 n = vecVN[aIndex[i][2] - 1];
				if (vertMat) {
					D3DXVec3TransformCoord(&p, &p, vertMat);
					D3DXVec3TransformNormal(&n, &n, vertMat);
				}
				vecVertex.push_back(ST_PNT_VERTEX(p, n, t));
			}
		}
	...
	cFrame * object = new cFrame;
	ID3DXMesh* pMesh;
	std::vector<cMtlTex*> materials;
	m_meshCreator.CreateMesh(&pMesh, materials);
	object->SetMesh(pMesh, materials);
	*meshObject = object;
...
void cObjLoader::LoadMtlLib( char* szFullPath )
{
	...
	while(!feof(fp))
	{
		...
		else if(szBuf[0] == 'n')
		{
			...
			m_meshCreator.AddMaterial(szMtlName, m_mapMtlTex[szMtlName]);
		}
		else if(szBuf[0] == 'K')
		{
