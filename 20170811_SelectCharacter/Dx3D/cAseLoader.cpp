...
cFrame* cAseLoader::Load( char* szFullPath )
{
	...
	while(true)
	{
		...
		else if (IsEqual(szToken, ID_GEOMETRY))
		{
			...
			cMeshCreateHelper meshCreator;
			meshCreator.AddFaces(pFrame->GetVertex());
			ID3DXMesh* pMesh;
			std::vector<cMtlTex*> materials;
			meshCreator.CreateMesh(&pMesh, materials);
			pFrame->SetMesh(pMesh, materials);
			if (pFrame) {
				pFrame->SetAnimTimeOffset(m_Firstframe*m_Ticksperframe);
				pFrame->SetAnimTimeFrameLength(m_Lastframe*m_Ticksperframe - m_Firstframe*m_Ticksperframe);
				pFrame->SetAnimTimeScale(m_FrameSpeed*m_Ticksperframe *0.001);
			}
			if(pRoot == NULL)
				pRoot = pFrame;
		}
...
void cAseLoader::ProcessMATERIAL_LIST()
{
	...
	do 
	{
		...
		else if (IsEqual(szToken, ID_MATERIAL)) 
		{
			...
			m_meshCreator.AddMaterial(std::to_string(nMtlRef), m_vecMtlTex[nMtlRef]);
