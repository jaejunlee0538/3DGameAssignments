...
void Bone::calculateCombinedTransformationMatrix(const D3DXMATRIX * parentMatrix)
{
	if (parentMatrix == NULL) {
		...
		combinedTransformationMatrix = TransformationMatrix * *parentMatrix;
	}
	Bone* sibling = static_cast<Bone*>(this->pFrameSibling);
	if (sibling) {
		sibling->calculateCombinedTransformationMatrix(parentMatrix);
	}

	Bone* child = static_cast<Bone*>(this->pFrameFirstChild);
	if (child) {
		child->calculateCombinedTransformationMatrix(&combinedTransformationMatrix);
	}
}
