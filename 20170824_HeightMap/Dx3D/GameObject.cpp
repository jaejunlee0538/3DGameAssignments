...
{
	setPosition(_position.x, y, _position.z);
}
...
{
	_speed = D3DXVECTOR3(vx, vy, vz);
}

void GameObject::setAngularSpeed(float vx, float vy, float vz)
{
	_angleSpeed = D3DXVECTOR3(vx, vy, vz);
}
...
void GameObject::UpdatePositionWithSpeed(float dt)
{
	...
	D3DXVECTOR3 deltaAngle = _angleSpeed * dt;
	D3DXVECTOR3 deltaPos = _speed * dt;
	rotateLocal(deltaAngle.x, deltaAngle.y, deltaAngle.z);
	translateLocal(deltaPos.x, deltaPos.y, deltaPos.z);
	_isDirty = true;
}

bool GameObject::isMoving() const
{
	return D3DXVec3LengthSq(&_angleSpeed) > 0.0001f || D3DXVec3LengthSq(&_speed) > 0.0001f;
}
