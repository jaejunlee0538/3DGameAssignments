...
void Grid::init(int nX, int nZ, float sizeX, float sizeZ)
{
	...
	for (int iX = -nX / 2; iX <= nX / 2; ++iX) {
		...
		//}
		_vertices.push_back(ST_PC_VERTEX(x, 0, zBottom, lineColor));
		_vertices.push_back(ST_PC_VERTEX(x, 0, -zBottom, lineColor));
		if (iX == 0) {
			_vertices.push_back(ST_PC_VERTEX(x, 0, zBottom, D3DCOLOR_XRGB(0,0,255)));
			_vertices.push_back(ST_PC_VERTEX(x, 0, -zBottom, D3DCOLOR_XRGB(0, 0, 255)));
		}
	}
	for (int iZ = -nZ / 2; iZ <= nZ / 2; ++iZ) {
		...
		_vertices.push_back(ST_PC_VERTEX(xLeft, 0, z, lineColor));
		_vertices.push_back(ST_PC_VERTEX(-xLeft, 0, z, lineColor));
	}

	_vertices.push_back(ST_PC_VERTEX(0, xLeft, 0, D3DCOLOR_XRGB(0, 255, 0)));
	_vertices.push_back(ST_PC_VERTEX(0, -xLeft, 0, D3DCOLOR_XRGB(0, 255, 0)));
