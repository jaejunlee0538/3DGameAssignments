	...
	m_nextCallbackKey(0)
{
	...
	pSkinnedMesh->m_pAnimController->CloneAnimationController(
		...
		&m_pAnimController);
	//
	//for (UINT i = 0; i < m_pAnimController->GetNumAnimationSets(); ++i) {
	//	ID3DXAnimationSet* animSet;
	//	m_pAnimController->GetAnimationSet(i, &animSet);
	//	LogDebugMessage("Anim %u : %s\n", i, animSet->GetName());
	//}
}
...
void cSkinnedMesh::UpdateAndRender(const D3DXMATRIXA16* matParent)
{
	if (m_pAnimController) {
		m_pAnimController->AdvanceTime(g_pTimeManager->GetDeltaTime(), NULL);
		D3DXTRACK_DESC trackDesc;
		m_pAnimController->GetTrackDesc(CURRENT_ANIM_TRACK, &trackDesc);
		if (trackDesc.Position > m_periodCurrentAnim) {
			CheckAndCallCallback(trackDesc.Position);
			trackDesc.Position -= m_periodCurrentAnim;
			m_pAnimController->SetTrackPosition(CURRENT_ANIM_TRACK, trackDesc.Position);
			m_nextCallbackKey = 0;
		}
		CheckAndCallCallback(trackDesc.Position);
	}
	if (m_pRootFrame) {
		Update(m_pRootFrame, matParent);
		Render(m_pRootFrame);
	}
}
...
void cSkinnedMesh::Update(ST_BONE* pCurrent, const D3DXMATRIXA16* pmatParent)
{
...
void cSkinnedMesh::SetAnimationIndex(int nIndex, const std::vector<AnimCallbackKey>& callbackKey)
{
	if (!m_pAnimController)
		return;
	...
	if (pAnimSet) {
		SetCurrentAnimationSet(pAnimSet, callbackKey);
		SAFE_RELEASE(pAnimSet);
	}
}

void cSkinnedMesh::SetAnimationByName(const std::string & name, const std::vector<AnimCallbackKey>& callbackKey)
{
	if (!m_pAnimController)
		return;
	LPD3DXANIMATIONSET pAnimSet = NULL;
	m_pAnimController->GetAnimationSetByName(name.c_str(), &pAnimSet);
	if (pAnimSet) {
		SetCurrentAnimationSet(pAnimSet, callbackKey);
		SAFE_RELEASE(pAnimSet);
	}
...
void cSkinnedMesh::CheckAndCallCallback(float animKeyTime)
{
	if (m_callbackKeys.empty() //callbackkey가 없거나
		|| !m_callbackKeyFunc //콜백 함수가 없거나
		|| m_nextCallbackKey >= m_callbackKeys.size())//모든 callbackkey이 호출되었거나
		return;
	float phase = animKeyTime / m_periodCurrentAnim;
	if (phase >= m_callbackKeys[m_nextCallbackKey].animPhase) {
		m_callbackKeyFunc(m_callbackKeys[m_nextCallbackKey].userData);
		++m_nextCallbackKey;
	}
}
...
void cSkinnedMesh::SetCurrentAnimationSet(LPD3DXANIMATIONSET animSet, const std::vector<AnimCallbackKey>& callbackKey)
{
	if (animSet) {
		m_pAnimController->SetTrackAnimationSet(0, animSet);
		m_periodCurrentAnim = animSet->GetPeriod();
		m_callbackKeys = callbackKey;
	}
}
